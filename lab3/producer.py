...
def send_data(file_path, topic, cls):
    with open(file_path, "r") as f:
        ...
        for row in reader:
            # Normalize column names to match model attributes
            normalized_row = {k.replace(' ', '_').replace('.', '_'): v for k, v in row.items()}
            print(f"Raw CSV row for {topic}: {normalized_row}")  # Debug CSV input
            obj = cls(**normalized_row)
